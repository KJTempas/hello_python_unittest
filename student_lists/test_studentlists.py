...
class TestStudentLists(TestCase):
    ...
    def test_remove_student_not_in_class_list_raises_StudentError(self):
        test_class = ClassList(2)
        test_class.add_student('Test Student1')
        test_class.add_student('Test Student2')
        test_class.add_student('Test Student3')
        with self.assertRaises(StudentError): #error should be raised because student is not on list
    ...
    def test_index_of_student_when_student_not_in_list(self):
        test_class = ClassList(3)
        test_class.add_student('Annie')
        test_class.add_student('Ellie')
        test_class.add_student('Barb')
        self.assertIsNone(test_class.index_of_student('Paul'))

    
    ## TODO write a test for your new is_class_full method when the class is full. 
    # use assertTrue.
    def test_is_class_full_when_class_full(self):
        test_class = ClassList(3) #class holds 3 people
        test_class.add_student('Nicole')
        test_class.add_student('Sierra')
        test_class.add_student('Quinn')
